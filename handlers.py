...
def api_register_user(*, email, name, passwd):  # 注册信息包括用户名邮箱与密码
    # 验证输入的正确性
    # 如果没输入name
    # s.strip(rm)函数表示删除s字符串中开头、结尾处，位于rm删除序列的字符
    # 如果rm为空时，默认删除空白字符
...
def api_create_blog(request, *, name, summary, content):
    check_admin(request)
    if not name or not name.strip():
        raise APIValueError('name', 'Name cannot be none!')
    if not summary or not summary.strip():
        raise APIValueError('summary', 'Summary can be none!')
    if not content or not content.strip():
        raise APIValueError('content', 'Content cannot be none!	')
